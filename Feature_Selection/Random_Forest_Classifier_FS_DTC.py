...
gscv.fit(X_train_dtc_df, Y_train)
...
rfc = RandomForestClassifier(n_estimators=50,max_samples=0.8,max_depth=5,criterion='gini',ccp_alpha=0.1)
rfc.fit(X_train_dtc_df, Y_train)
Y_pred_rfc=rfc.predict(X_test_dtc_df)
...
pickle.dump(rfc, open('Random_Forest_Classifier_FS_DTC.pkl','wb'))
...
accuracy_train = cross_val_score(rfc, X_train_dtc_df, Y_train, cv=5)
